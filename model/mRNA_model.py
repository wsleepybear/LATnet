...
class LightningAttentionModel(nn.Module):
    def __init__(self):
        super(LightningAttentionModel, self).__init__()
        self.attention = lightning_attn_func

    def forward(self, q, k, v, s):
        o = self.attention(q, k, v, s)
...
class LightningTransformer(nn.Module):
    def __init__(self, num_heads, sequence_length, embedding_dim):
        ...
        self.lightningAttention = LightningAttentionModel()
        ...
        self.positional_encoding = create_positional_encoding(6000, 640)
...
class LightningTransformerseq(nn.Module):
    def __init__(self, num_heads, sequence_length, embedding_dim):
        ...
        self.norm = nn.LayerNorm(head_dim)
        # You may want to add additional layers or components of the Transformer here
        self.drop = nn.Dropout(0.4)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.LightningTransformer = LightningTransformer(10, 6000, 640)
        self.LightningTransformerseq = LightningTransformerseq(4, 6000, 64)
        self.fc = nn.Sequential(
            nn.Linear(8, 32),  
            nn.LeakyReLU(),  
            nn.Linear(32, 64),  
